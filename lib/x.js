...
// Check whether user follows @handle (X API v2, paid follows.read).
...
// Check whether user liked a tweet (X API v2, POST-like check via liking_users).
// Endpoint /2/tweets/:id/liking_users returns the users who liked — we page
// through it looking for the user's own id. Note: this endpoint requires a
// paid tier (or app-level) bearer; on free tier it may 403 → returns false.
async function checkLiked(accessToken, userId, statusId) {
  if (cfg.mock) return true;
  let pagination_token = undefined;
  for (let page = 0; page < 20; page++) {
    let url = `${PROVIDER_BASE}/tweets/${statusId}/liking_users?max_results=100${pagination_token ? '&pagination_token=' + pagination_token : ''}`;
    const res = await fetch(url, { headers: { Authorization: `Bearer ${accessToken}` } });
    if (!res.ok) return false;
    const data = await res.json();
    if ((data.data || []).some(u => u.id === userId)) return true;
    pagination_token = data.meta?.next_token;
    if (!pagination_token) break;
  }
  return false;
...
// Check whether user reposted (retweeted) a tweet (X API v2).
async function checkReposted(accessToken, userId, statusId) {
  if (cfg.mock) return true;
  let pagination_token = undefined;
  for (let page = 0; page < 20; page++) {
    let url = `${PROVIDER_BASE}/tweets/${statusId}/retweeted_by?max_results=100${pagination_token ? '&pagination_token=' + pagination_token : ''}`;
    const res = await fetch(url, { headers: { Authorization: `Bearer ${accessToken}` } });
    if (!res.ok) return false;
    const data = await res.json();
    if ((data.data || []).some(u => u.id === userId)) return true;
...
module.exports = { cfg, buildAuthorizeUrl, exchangeCode, checkFollow, checkLiked, checkReposted };
